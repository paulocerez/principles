...
const components: PortableTextComponents = {
  ...
  block: {
    h2: ({ children }) => (
      <h2 className="mt-12 mb-3 text-[24px] font-normal tracking-[-0.02em] text-ink">{children}</h2>
    ),
    h3: ({ children }) => (
      <h3 className="mt-9 mb-2 text-[19px] font-normal tracking-[-0.01em] text-ink">{children}</h3>
    ),
    blockquote: ({ children }) => (
      <blockquote
